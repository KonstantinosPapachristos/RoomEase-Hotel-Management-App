...
public class ManagerDashboardController {
    ...
    // FXML TableView and Columns for Bookings
    @FXML
    ...
    // DatePickers for Report Generation
    @FXML
    ...
    // Load RoomStatus data from the database
    ...
    private double calculatePrice(String roomType, LocalDate checkIn, LocalDate checkOut, boolean isEvent) {
        ...
        if (isSummer(checkIn)) nightlyPrice *= 1.2;         // 20% markup for summer
        if (isChristmas(checkIn)) nightlyPrice *= 1.3;     // 30% markup for Christmas
        if (isOrthodoxEaster(checkIn)) nightlyPrice *= 1.25; // 25% markup for Orthodox Easter
        if (isEvent) nightlyPrice *= 1.4;                 // 40% markup for events

        long nights = java.time.temporal.ChronoUnit.DAYS.between(checkIn, checkOut);
        return nightlyPrice * nights; // Calculate total price
    }

    private double calculateAmountPaid(String bookingType, double totalPrice, LocalDate checkInDate) {
        if ("Prepaid".equals(bookingType)) {
            if (LocalDate.now().isBefore(checkInDate.minusDays(90))) {
                return totalPrice * 0.5; // 50% upfront for prepaid if 90+ days in advance
            } else {
                return totalPrice; // 100% if within 90 days
            }
        } else if ("Traditional".equals(bookingType)) {
            return totalPrice * 0.2; // 20% upfront for traditional bookings
        }
    ...
    private boolean isOrthodoxEaster(LocalDate date) {
        LocalDate easterSunday = LocalDate.of(date.getYear(), 4, 8);
    ...
    public void generateReport() {
        ...
        if (startDate == null || endDate == null) {
            showAlert("Invalid Input", "Please select both a start date and an end date.");
            return;
        }
        if (startDate.isAfter(endDate)) {
            showAlert("Invalid Input", "Start date cannot be after the end date.");
            return;
        }

        // Fetch total revenue from the DAO
        double totalRevenue = bookingDAO.getRevenueForDateRange(startDate, endDate);

        // Display the revenue in a dialog
        Alert reportDialog = new Alert(Alert.AlertType.INFORMATION);
        reportDialog.setTitle("Revenue Report");
        reportDialog.setHeaderText("Revenue Report for Selected Dates");
        reportDialog.setContentText("Total Revenue: $" + String.format("%.2f", totalRevenue));
    ...
    public void viewStatistics() {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/roomease/statistics.fxml"));
            Parent root = loader.load();

            // Get the current stage and replace the scene
            Stage stage = (Stage) bookingsTable.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.setMaximized(true); // Ensure it remains maximized
        } catch (Exception e) {
            e.printStackTrace();
            showAlert("Error", "Unable to load the Statistics page.");
        }
    }
